...
- Config is `~/.config/lane/config.json`. No automatic live reload; after editing it directly, run `lane --rediscover` (or restart the daemon) so the running process picks it up.
